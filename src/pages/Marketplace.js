...
const Marketplace = () => {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then(r => r.json())
      .then(data => {
        if (tab === 'shops') setShops(Array.isArray(data) ? data : []);
        else if (tab === 'food') setShops(Array.isArray(data) ? data : []);
    ...
    <div style={{ background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1B4332', padding: '24px 0 0' }}>
        <div className="container">
          ...
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem', marginBottom: '16px' }}>
            Pezani zinthu mu Zambia — Find anything, anywhere
          </p>
      ...
      <div className="container" style={{ padding: '16px 28px' }}>
        ...
        <div style={{ marginTop: '32px', background: '#1B4332', borderRadius: '24px', padding: '28px 24px', textAlign: 'center' }}>
          <div style={{ fontSize: '2rem', marginBottom: '8px' }}>💼</div>
          <h3 style={{ color: '#FFD966', marginBottom: '8px', fontSize: '1.2rem' }}>Spitsa pa TULI!</h3>
