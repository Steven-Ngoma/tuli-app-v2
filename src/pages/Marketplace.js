...
const Marketplace = () => {
  ...
  const emptyMsg = tab === 'products' ? 'No shops found.' : 'No services listed yet.';
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px' }} className="products-grid">
            {filtered.map(shop => {
              ...
              return (
                ...
                >
                  {/* Cover / Logo area */}
                  <div style={{ width: '100%', height: '100px', background: '#1B4332', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                    {(isService ? shop.cover_image : shop.logo_url)
                      ? <img src={isService ? shop.cover_image : shop.logo_url} alt={shop.shop_name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      : <span style={{ fontSize: '2.5rem' }}>{isService ? '🍽️' : '🏪'}</span>}
                    {/* Logo circle overlay for product shops */}
                    {!isService && shop.logo_url && (
                      <div style={{ position: 'absolute', bottom: '-20px', left: '16px', width: '44px', height: '44px', borderRadius: '50%', border: '3px solid #fff', overflow: 'hidden', background: '#fff' }}>
                        <img src={shop.logo_url} alt="logo" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      </div>
                    )}
                  </div>
                  <div style={{ padding: !isService && shop.logo_url ? '28px 10px 10px' : '10px', flex: 1, display: 'flex', flexDirection: 'column' }}>
